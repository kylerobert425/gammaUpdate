...
testerList <- "~/Desktop/Memfault Test/BlackBox/Gamma Testers with UUIDs.xlsx"
...
results <- data.frame(matrix(nrow = 0, ncol = length(result_names)))
...
for(i in seq(from=1, to=length(files), by=1)) {
    ...
    x <- '~/Desktop/Memfault Test/BlackBox/Update1/GammaUpdate1_group1_682719B2E8A8_682719B2E8A81675205852_230131_1557.xlsx'
    ...
    output <- data.frame(c(uuid, cookid, size, date, time,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA))
    ...
    output[22] <- error_vals$errors[mat_nonzero]
    ...
    #### OUTPUT STATS ####
    if(saveOutput){
      results <- rbind(results, t(output))
    }
